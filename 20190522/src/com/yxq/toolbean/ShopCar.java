        ...
        if (single != null) {
            ...
            if (buylist.size() == 0) {                                                //如果buylist中不存在任何商品
                ...
                for (; i < buylist.size(); i++) {                                        //遍历buylist集合对象，判断该集合中是否已经存在当前要添加的商品
                    ...
                    if (temp.getId() == single.getId()) {                //判断从buylist集合中获取的当前商品的名称是否与要添加的商品的名称相同
                        //如果相同，说明已经购买了该商品，只需要将商品的购买数量加1
                        if (temp.getNum() >= single.getNum())
                            message = "数量超出库存";
                        else
